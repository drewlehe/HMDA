...
HMDA_DIR = "/Users/andrew.lehe/Documents/HMDA Data/old_hmdas"
CROSSWALK_PATH = "TRACT_ZIP_122022.xlsx"
OUTPUT_DIR = "/Users/andrew.lehe/Documents/HMDA Data/old_hmda_outputs"
...
def map_action(x: str):
    x = str(x).strip().lower()
    if x in {"loan originated", "application approved but not accepted"}:
        return "Approved"
    if x == "application denied by financial institution":
        return "Denied"
    return None


def map_purchaser(x: str) -> str:
    x = str(x).strip().lower()
    if "commercial bank" in x or "savings bank" in x or "savings association" in x:
        return "Commercial Bank"
    if "ginnie mae" in x or "gnma" in x or "fha" in x:
        return "FHA (Ginnie Mae)"
    if "fannie mae" in x or "fnma" in x:
        return "Fannie Mae"
    if "freddie mac" in x or "fhlmc" in x:
        return "Freddie Mac"
    return "Other"


def clean_hmda_chunk(chunk: pd.DataFrame) -> pd.DataFrame:
    chunk["state_fips_clean"] = (
        pd.to_numeric(chunk["state_code"], errors="coerce")
        .astype("Int64")
        .astype(str)
        .str.zfill(2)
    )

    chunk["county_fips_clean"] = (
        pd.to_numeric(chunk["county_code"], errors="coerce")
        .astype("Int64")
        .astype(str)
        .str.zfill(3)
    )

    chunk["tract_code_clean"] = (
        np.round(pd.to_numeric(chunk["census_tract_number"], errors="coerce") * 100)
        .astype("Int64")
        .astype(str)
        .str.zfill(6)
    )
    ...
    chunk["year"] = pd.to_numeric(chunk["as_of_year"], errors="coerce").astype("Int64")
    chunk["loan_amount_000s_num"] = pd.to_numeric(chunk["loan_amount_000s"], errors="coerce")
    chunk["applicant_income_000s_num"] = pd.to_numeric(chunk["applicant_income_000s"], errors="coerce")

    chunk["population_num"] = pd.to_numeric(chunk["population"], errors="coerce")
    chunk["minority_population_num"] = pd.to_numeric(chunk["minority_population"], errors="coerce")
    chunk["hud_median_family_income_num"] = pd.to_numeric(chunk["hud_median_family_income"], errors="coerce")
    chunk["tract_to_msamd_income_num"] = pd.to_numeric(chunk["tract_to_msamd_income"], errors="coerce")
    chunk["owner_units_num"] = pd.to_numeric(chunk["number_of_owner_occupied_units"], errors="coerce")
    chunk["total_units_num"] = pd.to_numeric(chunk["number_of_1_to_4_family_units"], errors="coerce")
...
def process_file(hmda_path: str, crosswalk: pd.DataFrame, output_path: str):
    ...
    reader = pd.read_csv(hmda_path, low_memory=False, chunksize=CHUNK_SIZE)
    ...
    for i, chunk in enumerate(reader, start=1):
        ...
        chunk["occupancy_group"] = chunk["owner_occupancy_name"].map(map_occupancy)
        chunk["decision_group"] = chunk["action_taken_name"].map(map_action)
        chunk["purchaser_group"] = chunk["purchaser_type_name"].map(map_purchaser)
        ...
        tract_zip["minority_population_alloc"] = (
            tract_zip["minority_population_num"] * tract_zip["tot_ratio"]
        ).clip(upper=tract_zip["population_alloc"])
...
def main():
    ...
    pattern = os.path.join(HMDA_DIR, "hmda_*_nationwide_all-records_labels.csv")
    files = sorted(glob.glob(pattern))

    if not files:
        raise FileNotFoundError(f"No HMDA files found matching: {pattern}")
    ...
    for hmda_path in files:
        ...
        # Extract year from filename like hmda_2007_nationwide_all-records_labels.csv
        year_match = re.search(r'hmda_(\d{4})_', basename)
